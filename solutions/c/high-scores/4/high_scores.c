    ...
    for (size_t i = 0; i < scores_len; i++) {
        ...
        for (int j = 0; j < top_n && uninserted; j++) {
            if (scores[i] > output[j]) {
                ...
                // Because we're writing to output[k + 1], we need to make sure that
                // we never set k to the last element of output, or we'll be writing
                // in memory we do not control and Bad Things could happen.
