...
size_t personal_top_three(const int32_t *scores, size_t scores_len,
    ...
    // First, write zeroes to output
    for (int i = 0; i < top_n; i++) { output[i] = 0; }

    int lowest_top_score = 0;
    int top_n_populated = 1;
    for (size_t i = 0; i < scores_len; i++) {
        // We can skip scores[i] if the top N has been populated and it's lower than the
        // lowest score in the top N...
        if ((top_n_populated < top_n) || (scores[i] > lowest_top_score)) {
            ...
            for (int j = 0; j < top_n && uninserted; j++) {
                if (scores[i] > output[j]) {
                    ...
                    for (int k = top_n - 2; k >= j; k--) {
                        output[k + 1] = output[k];
                    }                
                    // insert the new top N score...
                    output[j] = scores[i];
                    lowest_top_score = scores[i] < lowest_top_score ? scores[i] : lowest_top_score;
                    top_n_populated++;
                    uninserted = 0;
                }
            }            
